...
const SUPPLIERS = [
  { name: 'Grossiste X', type: 'grossiste', siret: '123 456 789 00012', contact: 'Marc Dubois', phone: '06 12 34 56 78', email: 'contact@grossistex.fr', address: '12 rue du Commerce, Lyon' },
  { name: 'Friperie du Centre', type: 'friperie', siret: '987 654 321 00045', contact: 'Sophie Martin', phone: '06 23 45 67 89', address: '45 rue principale, Marseille' },
  { name: 'Déstockage Pro', type: 'destockeur', siret: '456 789 123 00078', contact: 'Karim B.', phone: '06 34 56 78 90', address: 'Zone industrielle, Lille' },
  { name: 'Vide-grenier Vincennes', type: 'vide-grenier', siret: null, contact: 'Bénévole', address: 'Place de Vincennes, Paris' },
  { name: 'Particulier — Leboncoin', type: 'particulier', siret: null, contact: 'Anonyme' },
  { name: 'Stock Luxe Paris', type: 'grossiste', siret: '321 654 987 00023', contact: 'Élodie Roux', phone: '06 78 90 12 34', address: '8 avenue Montaigne, Paris' },
]
...
const PARCEL_STATUSES = ['A_PREPARER', 'A_IMPRIMER', 'A_DEPOSER', 'EN_TRANSIT', 'LIVRE', 'PROBLEME']
...
// Re-seed route — ADMIN ONLY
// Reinitializes the database with demo data, assigned to the current admin user.
export async function POST() {
  try {
    const admin = await requireAdmin()

    // Wipe business data only (never users — they're managed separately).
    // Attributes are shared/global, so we wipe them entirely too and re-seed.
    // StockItems/Suppliers/Expenses: wipe ALL of them (not just current user's)
    // because the admin is intentionally resetting the demo dataset, and
    // legacy rows from previous seeds may have userId=null.
    ...
    const createdSuppliers = await Promise.all(
      SUPPLIERS.map(s => db.supplier.create({ data: { ...s, userId: admin.id } }))
    )

    // 2. Stock items
    const now = new Date()
    const createdItems = []
    for (let i = 0; i < STOCK_SEED.length; i++) {
      const s = STOCK_SEED[i]
      ...
      const item = await db.stockItem.create({
        data: {
          ...
          purchaseInvoiceNumber: `FAC-2026-${String(i + 1).padStart(3, '0')}`,
          purchasePaymentMethod: ['especes', 'carte_bancaire', 'virement', 'cheque', 'paypal'][i % 5],
          warehouse: i % 2 === 0 ? 'Entrepôt Principal' : 'Entrepôt Annex',
          rack: `Rack ${String.fromCharCode(65 + (i % 4))}`,
          shelf: `${(i % 4) + 1}`,
          bin: `${(i % 12) + 1}`,
          status,
          platform: status === 'VENDU' ? pick(PLATFORMS, i) : null,
          platforms: status === 'PUBLIE' || status === 'PRET_A_PUBLIER'
            ? JSON.stringify([pick(PLATFORMS, i), pick(PLATFORMS, i + 1)])
            : '[]',
    ...
    for (let i = 0; i < 10; i++) {
      ...
      await db.sale.create({
        data: {
          ...
          parcelStatus: pick(PARCEL_STATUSES, i + 1),
          notes: '',
          userId: admin.id,
          invoiceNumber: `F-${new Date(saleDate).getFullYear()}-${String(i + 1).padStart(3, '0')}`,
        },
      })
    ...
    // 4. Attributes (global — shared between all users)
    const attrsByType: Record<string, typeof DEFAULT_ATTRS> = {}
    ...
    // 6. Purchases (achats hors stock)
    await db.purchase.deleteMany()
    const PURCHASES = [
      { designation: 'Cartons d\'emballage x50', category: 'emballage', amount: 28.00, invoiceNumber: 'FAC-EMB-001', paymentMethod: 'carte_bancaire', supplierId: createdSuppliers[2].id },
      { designation: 'Rouleau papier bulle', category: 'emballage', amount: 12.50, invoiceNumber: 'FAC-EMB-002', paymentMethod: 'especes' },
      { designation: 'Imprimante étiquettes', category: 'materiel', amount: 89.00, invoiceNumber: 'FAC-MAT-001', paymentMethod: 'carte_bancaire', supplierId: createdSuppliers[0].id },
      { designation: 'Stylos + surligneurs', category: 'fourniture', amount: 8.50, paymentMethod: 'especes' },
      { designation: 'Balance de précision', category: 'outil', amount: 45.00, invoiceNumber: 'FAC-OUT-001', paymentMethod: 'virement', supplierId: createdSuppliers[2].id },
    ]
    ...
    return NextResponse.json({
      success: true,
      message: 'Base réinitialisée avec des données démo',
      stats: {
        suppliers: createdSuppliers.length,
        stockItems: createdItems.length,
        sales: 10,
        expenses: EXPENSES.length,
        attributes: DEFAULT_ATTRS.length,
      },
    })
  } catch (error) {
    ...
    return NextResponse.json({ error: 'Erreur lors du re-seed' }, { status: 500 })
  }
}
