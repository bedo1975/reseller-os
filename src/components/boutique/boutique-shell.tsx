...
import { TrackingScript } from '@/components/boutique/tracking-script'
      ...
      <div className="text-white text-xs" style={{ backgroundColor: topbarBgColor }}>
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-4">
            <Link href="/cgv" className="hover:text-blue-200 transition-colors">CGV</Link>
            <span>·</span>
          ...
          <div className="max-w-7xl mx-auto px-4">
              ...
              <nav className="hidden lg:flex items-center gap-1">
                <Link
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
                    pathname === '/' ? 'text-[#007bff] bg-blue-50' : 'text-gray-700 hover:text-[#007bff] hover:bg-gray-50'
                  }`}
                >
                  Accueil
                </Link>
                {(navMenuItems ? navMenuItems : categories).map((item: any) => {
                  ...
                  const url = item.url || `/categorie/${item.slug}`
                      ...
                      {subcats.length > 0 && hoveredCat === (slug || url) && (
                        <div className="absolute top-full left-0 mt-0 min-w-[200px] bg-white border border-gray-200 rounded-md shadow-lg py-2 z-50">
                          {subcats.map((s: any) => (
                            <Link
                              key={s.code}
                              href={`/categorie/${slug}?subcat=${s.code}`}
            ...
            {mobileMenuOpen && (
              <nav className="lg:hidden border-t border-gray-200 py-2 space-y-1">
                {/* Mobile search */}
                <form action="/recherche" className="px-4 py-2">
                    ...
                    {c.subcategories.length > 0 && (
                      <div className="pl-8">
                        {c.subcategories.map(s => (
                          <Link
                            key={s.code}
                            href={`/categorie/${c.slug}?subcat=${s.code}`}
                            onClick={() => setMobileMenuOpen(false)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  <li><Link href="/" className="text-blue-100 hover:text-white transition-colors">Tous les produits</Link></li>
                  {categories.map(c => (
                    <li key={c.slug}>
                      <Link href={`/categorie/${c.slug}`} className="text-blue-100 hover:text-white transition-colors">
                        {c.label}
                      </Link>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  {footerInfosLinks.length > 0 ? (
                    ...
                    <>
                      <li><Link href="/cgv" className="text-blue-100 hover:text-white transition-colors">CGV</Link></li>
                      <li><Link href="/mentions-legales" className="text-blue-100 hover:text-white transition-colors">Mentions légales</Link></li>
                      <li><Link href="/panier" className="text-blue-100 hover:text-white transition-colors">Mon panier</Link></li>
                      <li><Link href="/contact" className="text-blue-100 hover:text-white transition-colors">Contact</Link></li>
                      <li><Link href="/admin" className="text-blue-100 hover:text-white transition-colors">Espace gestion</Link></li>
                    </>
                  )}
                </ul>
              </div>
